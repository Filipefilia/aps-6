...
def load_known_faces():
    ...
    for user_folder in os.listdir(DATASET_DIR):
        ...
        if os.path.isdir(user_dir_path):
            # Extract user name from folder name "access_levelX-username"
            try:
            ...
            if os.path.exists(encodings_file):
                ...
                print(f"[INFO] Loaded {len(encodings)} encodings for user: {user_name}")
            else:
...
def recognize_faces_in_video():
    ...
    while True:
        ...
        frame, name = process_frame_for_recognition(frame, known_face_encodings, known_face_names)
